...
def update_user(request):
    try:
        if request.method != 'POST' and request.user.isAdmin == False or request.user.can_add_employees == False:
...
def get_employee_leave_data(request):
    ...
    leave_data = Leave.objects.values('user__id', 'user__username','user__emplyeeIdentficationCode', 'start_date', 'end_date')
